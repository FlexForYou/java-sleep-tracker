...
public class LotSessionsWithBadSleep implements Function<List<SleepingSession>, SleepAnalysisResult> {
    ...
    public SleepAnalysisResult apply(List<SleepingSession> sessions) {
        ...
        Long badSleepQuality = sessions.stream()
                .filter(session -> session.getSleepQuality() == SleepQuality.BAD)
                .count();

        return new SleepAnalysisResult("Количество сессий с плохим качеством сна", badSleepQuality);
    }
}
